                ...
                <div class="content">
                  <div class="content-left">
                    ...
                    <el-form-item prop="status" label="状态">
                      <div class="module-border">
                        <el-radio-group v-model="temp1.status">
                          <el-radio
                            v-for="item in templateStatusFilterData"
                            :key="item.value"
                            :label="item.value"
                            :value="item.value"
                            @change="setModelStatusValue(item.value)"
                            >{{ item.text }}</el-radio
                          >
                        </el-radio-group>
                      </div>
                    ...
                    <el-form-item prop="domain" label="领域范围">
                      <div class="module-border">
                        <el-radio-group v-model="temp1.domain">
                          <el-radio
                ...
                >
                  <div class="content-left">
                    <el-upload
                      ...
                      :on-change="setModelWebsiteInfoLogo"
                  ...
                  <div class="content-right">
                    <el-upload
                      ...
                      :auto-upload="false"
                      :show-file-list="false"
                      :on-change="setModelWebsiteInfoPbqrUrl"
                      accept="image/jpg, image/jpeg, image/png"
                    >
                      <img
                        v-if="temp4.pbqrUrl"
                    ...
                    <el-upload
                      ...
                      :auto-upload="false"
                      :show-file-list="false"
                      :on-change="setModelWebsiteInfoBglgUrl"
                      accept="image/jpg, image/jpeg, image/png"
                    >
                      <img
                        v-if="temp4.bglgUrl"
...
export default {
  ...
  mounted() {
    ...
    getTemplateStatus().then((res) => {
      this.templateStatusFilterData = res.data;
    });
  ...
  methods: {
    getParams() {
      this.tplId = getStore("templateId");
      if (this.tplId === null || this.tplId === "") {
        this.viewStatus = "create";
      } else {
        this.viewStatus = "update";
        this.getTemplate();
      }
    },
    // 得到模板基本配置
    getTemplate() {
      selectTemplateById(this.tplId).then((res) => {
        if (res.data != null) {
          this.temp1 = res.data;
        }
      });
    ...
    // 获取菜单列表
    getMenuList() {
      this.menuTemp.objId = this.tplId;
    ...
    setDomainValue(event) {
      this.temp1.domain = event;
    },
    // 提交基本设置
    submitTemp1() {
      this.$refs["dataForm"].validate((valid) => {
        if (valid) {
          if (this.viewStatus === "create") {
            addTemplate(this.temp1).then((res) => {
              this.saveSuccessMsg();
              this.tplId = res.data;
              setStore("templateId", res.data);
              this.getTemplate();
            });
          } else {
            updateTemplate(this.temp1).then(() => {
              this.saveSuccessMsg();
              this.getTemplate();
            });
          }
    ...
    submitRoleFormData() {
      this.$refs["roleDataForm"].validate((valid) => {
        if (valid) {
          this.roleTemp.objId = this.tplId;
    ...
    appStatusFilters(value) {
      let data = "";
      this.templateStatusFilterData.map(function(item) {
        if (value === item.value) {
          data = item.text;
        }
      });
    ...
    setModelWebsiteInfoBglgUrl(file) {
      if (this.checkImg(file)) {
        var reader = new FileReader();
        let imgFile;
        var that = this;
    ...
    setModelWebsiteInfoPbqrUrl(file) {
      if (this.checkImg(file)) {
        var reader = new FileReader();
        let imgFile;
        var that = this;
    ...
    getUserList() {
      selectUserByAppId(this.tplId).then((res) => {
        this.userTableData = res.data;
      });
    },
    getPrincipalList() {
      selectAcl({ objId: this.tplId }).then((res) => {
        this.principalDatas = res.data.list;
      });
    },
    dialogCloseBefore() {
    ...
    getAppRoleList() {
      selectRoleByAppId(this.tplId).then((res) => {
    ...
    submitPrincipalCreate() {
      addAcl(this.tplId, this.principalType, this.principalData).then(() => {
        this.saveSuccessMsg();
        this.getPrincipalList();
